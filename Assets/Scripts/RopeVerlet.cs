...
public class RopeVerlet : MonoBehaviour
{
    ...
    private void SimulateRope()
    {
        for (int i = 0; i < _ropeSegments.Count; i++)
        {
            ...
            Vector2 velocity = (segment.currentPos - segment.oldPos) * _dampingFactor;

            segment.oldPos = segment.currentPos;
            segment.currentPos += velocity;
            segment.currentPos += _gravityForce * Time.fixedDeltaTime;
    ...
    private void ApplyConstraints()
    {
        ...
        for (int i = 0; i < _numOfRopeSegments - 1; i++)
        {
            ...
            float dist = (currentSegment.currentPos - nextSegment.currentPos).magnitude;
